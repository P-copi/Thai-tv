...
public class MainActivity extends Activity {
    ...
    class SwipeOverlay extends View {
        ...
        SwipeOverlay(Context c,boolean l){super(c);left=l;setLayerType(View.LAYER_TYPE_SOFTWARE,null);setVisibility(INVISIBLE);}
        void setValue(float v){value=Math.max(0,Math.min(1,v));setVisibility(VISIBLE);invalidate();}
        @Override protected void onDraw(Canvas c){float w=getWidth(),h=getHeight(),barW=Math.max(7,Math.min(12,w*.07f));float top=h*.22f,bottom=h*.78f,x=left?w*.06f:w*.94f-barW;p.setColor(Color.argb(110,255,255,255));c.drawRoundRect(x,top,x+barW,bottom,barW,barW,p);p.setColor(red);float fill=top+(bottom-top)*(1-value);c.drawRoundRect(x,fill,x+barW,bottom,barW,barW,p);p.setTextAlign(Paint.Align.CENTER);p.setTextSize(Math.max(24,w*.045f));p.setTypeface(Typeface.DEFAULT_BOLD);p.setColor(Color.WHITE);c.drawText(Math.round(value*100)+"%",left?w*.13f:w*.87f,h*.86f,p);}
    }

    class FullscreenFrameLayout extends FrameLayout {
        float downX,downY; boolean adjusting; final AudioManager audio=(AudioManager)getSystemService(AUDIO_SERVICE); SwipeOverlay leftOverlay,rightOverlay;
        ...
        @Override public boolean onInterceptTouchEvent(MotionEvent e){switch(e.getActionMasked()){case MotionEvent.ACTION_DOWN:downX=e.getX();downY=e.getY();adjusting=false;break;case MotionEvent.ACTION_MOVE:float dx=e.getX()-downX,dy=e.getY()-downY;if(Math.abs(dy)>28&&Math.abs(dy)>Math.abs(dx)*1.15f){adjusting=true;return true;}break;case MotionEvent.ACTION_UP:case MotionEvent.ACTION_CANCEL:adjusting=false;break;}return super.onInterceptTouchEvent(e);}
        @Override public boolean onTouchEvent(MotionEvent e){if(!adjusting)return true;if(e.getActionMasked()==MotionEvent.ACTION_MOVE||e.getActionMasked()==MotionEvent.ACTION_UP){boolean right=e.getX()>getWidth()/2f;if(right){int max=audio.getStreamMaxVolume(AudioManager.STREAM_MUSIC),cur=audio.getStreamVolume(AudioManager.STREAM_MUSIC);int target=(int)Math.round(cur+(downY-e.getY())/getHeight()*1.5f*max);target=Math.max(0,Math.min(max,target));audio.setStreamVolume(AudioManager.STREAM_MUSIC,target,0);rightOverlay.setValue(max==0?0:target/(float)max);}else{WindowManager.LayoutParams p=getWindow().getAttributes();float cur=p.screenBrightness;if(cur<0)cur=.5f;float target=Math.max(.05f,Math.min(1f,cur+(downY-e.getY())/getHeight()*1.5f));p.screenBrightness=target;getWindow().setAttributes(p);leftOverlay.setValue(target);}if(e.getActionMasked()==MotionEvent.ACTION_UP)new Handler().postDelayed(()->{leftOverlay.setVisibility(INVISIBLE);rightOverlay.setVisibility(INVISIBLE);},900);return true;}return true;}
    }

    void enterFull(View v,WebChromeClient.CustomViewCallback cb){if(customView!=null){cb.onCustomViewHidden();return;}customView=v;customCallback=cb;v.setBackgroundColor(Color.BLACK);fullFrame=new FullscreenFrameLayout(this);fullFrame.addView(v,new FrameLayout.LayoutParams(-1,-1));setContentView(fullFrame);setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);hideBars();fullFrame.setSystemUiVisibility(5894|View.SYSTEM_UI_FLAG_IMMERSIVE_STICKY);}
    void enterPageFull(){if(pageFullscreen||customView!=null||web==null)return;ViewParent p=web.getParent();if(p instanceof ViewGroup)((ViewGroup)p).removeView(web);fullFrame=new FullscreenFrameLayout(this);fullFrame.addView(web,new FrameLayout.LayoutParams(-1,-1));setContentView(fullFrame);pageFullscreen=true;setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);hideBars();fullFrame.setSystemUiVisibility(5894|View.SYSTEM_UI_FLAG_IMMERSIVE_STICKY);}
    void exitPageFull(){if(!pageFullscreen)return;pageFullscreen=false;if(fullFrame!=null){fullFrame.removeView(web);fullFrame=null;}root.addView(web,2,webParams);setContentView(root);setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED);showBars();load(HOME);}
    void exitFull(){if(customView==null)return;View old=customView;customView=null;if(customCallback!=null){customCallback.onCustomViewHidden();customCallback=null;}fullFrame=null;setContentView(rootView);setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED);showBars();old.setSystemUiVisibility(View.SYSTEM_UI_FLAG_VISIBLE);load(HOME);}
    ...
    void buildUi(){
        ...
        web.setWebViewClient(new WebViewClient(){
            @Override public void onPageFinished(WebView v,String u){super.onPageFinished(v,u);if(!restoring&&!pageFullscreen&&!customView&&isWatchUrl(u)){new Handler().postDelayed(()->{if(!pageFullscreen&&!customView)enterPageFull();},250);}}
        ...
        LinearLayout nav=new LinearLayout(this);nav.setGravity(Gravity.CENTER);nav.setPadding(4,5,4,5);nav.setBackgroundColor(Color.rgb(20,20,20));TextView home=label("⌂\nหน้าแรก",12),back=label("‹\nย้อนกลับ",12),forward=label("›\nถัดไป",12),yt=label("▶\nYouTube",12);yt.setTextColor(red);home.setOnClickListener(v->load(HOME));back.setOnClickListener(v->{if(web.canGoBack())web.goBack();});forward.setOnClickListener(v->{if(web.canGoForward())web.goForward();});yt.setOnClickListener(v->openYouTubeApp());for(TextView t:new TextView[]{home,back,forward,yt})nav.addView(t,new LinearLayout.LayoutParams(0,58,1));root.addView(nav);setContentView(root);
    }
