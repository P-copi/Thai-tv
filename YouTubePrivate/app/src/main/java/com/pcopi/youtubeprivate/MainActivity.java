...
public class MainActivity extends Activity {
    WebView web; EditText search; View customView; WebChromeClient.CustomViewCallback customCallback; View appRoot;
    ...
    void enterFull(View v,WebChromeClient.CustomViewCallback cb){
        ...
        hideBars();
        v.setOnSystemUiVisibilityChangeListener(x->hideBars());
    }
    void exitFull(){
        if(customView==null)return;
        View old=customView;customView=null;
        if(customCallback!=null){WebChromeClient.CustomViewCallback c=customCallback;customCallback=null;c.onCustomViewHidden();}
        setContentView(appRoot);
        setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED);
        showBars();
    }
    void buildUi(){
        LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setBackgroundColor(Color.rgb(15,15,15));appRoot=root;
        ...
        root.addView(web,new LinearLayout.LayoutParams(-1,0,1));LinearLayout nav=new LinearLayout(this);nav.setGravity(Gravity.CENTER);nav.setPadding(4,5,4,5);nav.setBackgroundColor(Color.rgb(20,20,20));TextView home=label("⌂\nหน้าแรก",12),back=label("‹\nย้อนกลับ",12),forward=label("›\nถัดไป",12),yt=label("▶\nYouTube",12),fs=label("⛶\nเต็มจอ",12);yt.setTextColor(red);home.setOnClickListener(v->load(HOME));back.setOnClickListener(v->{if(web.canGoBack())web.goBack();});forward.setOnClickListener(v->{if(web.canGoForward())web.goForward();});yt.setOnClickListener(v->openYouTubeApp());fs.setOnClickListener(v->{if(customView!=null)exitFull();else web.evaluateJavascript("(function(){var e=document.querySelector('video');if(e){if(e.requestFullscreen)e.requestFullscreen();else if(e.webkitEnterFullscreen)e.webkitEnterFullscreen();}})()",null);});for(TextView t:new TextView[]{home,back,forward,yt,fs})nav.addView(t,new LinearLayout.LayoutParams(0,58,1));root.addView(nav);setContentView(root);
    }
