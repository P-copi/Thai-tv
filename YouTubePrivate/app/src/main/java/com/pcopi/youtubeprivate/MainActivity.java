...
public class MainActivity extends Activity {
    ...
    class FullscreenFrameLayout extends FrameLayout {
        ...
        @Override public boolean onTouchEvent(MotionEvent e){if(!adjusting)return true;if(e.getActionMasked()==MotionEvent.ACTION_MOVE||e.getActionMasked()==MotionEvent.ACTION_UP){boolean right=e.getX()>getWidth()/2f;float delta=(downY-e.getY())/Math.max(1f,getHeight());if(right){int max=audio.getStreamMaxVolume(AudioManager.STREAM_MUSIC);int target=Math.round(startVolume+delta*0.65f*max);target=Math.max(0,Math.min(max,target));audio.setStreamVolume(AudioManager.STREAM_MUSIC,target,0);rightOverlay.setValue(max==0?0:target/(float)max);}else{float target=Math.max(.05f,Math.min(1f,startBrightness+delta*0.80f));WindowManager.LayoutParams p=getWindow().getAttributes();p.screenBrightness=target;getWindow().setAttributes(p);leftOverlay.setValue(target);}return true;}return true;}
    }
