...
public class MainActivity extends ComponentActivity {
    ...
    private TextView tv(String text, float size) {
        TextView t = new TextView(this); t.setText(text); t.setTextSize(size); t.setTextColor(0xffeeeeee); t.setGravity(Gravity.CENTER_VERTICAL); t.setPadding(dp(16),0,dp(16),0); return t;
    }
    private Button btn(String text) {
        Button b = new Button(this); b.setText(text); b.setTextSize(14); return b;
    }

    private void buildUi() {
        drawer = new DrawerLayout(this); drawer.setBackgroundColor(0xff101010);
        ...
        TextView search = tv("⌕",28); search.setGravity(Gravity.CENTER); bar.addView(search,new LinearLayout.LayoutParams(dp(52),dp(56))); search.setOnClickListener(v->searchDialog());
        root.addView(bar);

        playerView = new PlayerView(this); playerView.setUseController(true); playerView.setShowBuffering(PlayerView.SHOW_BUFFERING_WHEN_PLAYING);
        playerView.setBackgroundColor(Color.BLACK);
        root.addView(playerView,new LinearLayout.LayoutParams(-1,dp(220)));

        LinearLayout controls = new LinearLayout(this); controls.setPadding(dp(8),dp(4),dp(8),dp(4));
        Button file = btn("ไฟล์ M3U"); Button net = btn("URL Playlist"); Button refresh = btn("↻ รีเฟรช");
        controls.addView(file,new LinearLayout.LayoutParams(0,dp(48),1)); controls.addView(net,new LinearLayout.LayoutParams(0,dp(48),1)); controls.addView(refresh,new LinearLayout.LayoutParams(0,dp(48),1)); root.addView(controls);
        ...
        listPanel = new LinearLayout(this); listPanel.setOrientation(LinearLayout.VERTICAL);
        recycler = new RecyclerView(this); recycler.setLayoutManager(new LinearLayoutManager(this)); adapter = new ChannelAdapter(); recycler.setAdapter(adapter); listPanel.addView(recycler,new LinearLayout.LayoutParams(-1,0,1));
        root.addView(listPanel,new LinearLayout.LayoutParams(-1,0,1));

        buildDrawer(); setContentView(drawer);
        player = new ExoPlayer.Builder(this).build(); playerView.setPlayer(player);
    }

    private void buildDrawer() {
        LinearLayout nav = new LinearLayout(this); nav.setOrientation(LinearLayout.VERTICAL); nav.setBackgroundColor(0xff181818); nav.setPadding(0,dp(30),0,dp(12));
        TextView head=tv("  Thai TV Player",22); head.setTypeface(null,1); head.setTextColor(0xffff9800); nav.addView(head,new LinearLayout.LayoutParams(-1,dp(65)));
        ...
        addNav(nav,"＋   เปิดไฟล์ M3U",this::pickFile);
        addNav(nav,"↗   เปิด URL Playlist",this::urlDialog);
        addNav(nav,"▶   เปิด URL สตรีม",this::streamDialog);
        addNav(nav,"⚙   ตั้งค่า",this::settingsDialog);
        addNav(nav,"ⓘ   เกี่ยวกับ",this::aboutDialog);
        DrawerLayout.LayoutParams lp=new DrawerLayout.LayoutParams(dp(300),-1); lp.gravity=Gravity.START; drawer.addView(nav,lp);
    }
    ...
    private void showChannels(ArrayList<Channel> data,String name){ title.setText(name); adapter.setData(data); status.setText(data.size()+" ช่อง"); }
    private void showChannels(List<Channel> data,String name){ title.setText(name); adapter.setData(data); status.setText(data.size()+" ช่อง"); }
    ...
    private void urlDialog(){ final EditText e=new EditText(this); e.setSingleLine(); e.setHint("https://.../playlist.m3u"); new AlertDialog.Builder(this).setTitle("URL Playlist").setView(e).setNegativeButton("ยกเลิก",null).setPositiveButton("เปิด",(d,w)->loadUrl(e.getText().toString().trim())).show(); }
    private void streamDialog(){ final EditText e=new EditText(this); e.setSingleLine(); e.setHint("https://.../stream.m3u8"); new AlertDialog.Builder(this).setTitle("เปิด URL สตรีม").setView(e).setNegativeButton("ยกเลิก",null).setPositiveButton("เล่น",(d,w)->play(new Channel("Network Stream","",e.getText().toString().trim()))).show(); }
    private void searchDialog(){ final EditText e=new EditText(this); e.setSingleLine(); e.setHint("ค้นหาช่อง"); new AlertDialog.Builder(this).setTitle("ค้นหาช่อง").setView(e).setNegativeButton("ปิด",null).setPositiveButton("ค้นหา",(d,w)->{String q=e.getText().toString().toLowerCase(Locale.ROOT); ArrayList<Channel> r=new ArrayList<>(); for(Channel c:channels) if(c.name.toLowerCase(Locale.ROOT).contains(q)) r.add(c); showChannels(r,"ผลการค้นหา");}).show(); }
    ...
    private ArrayList<Channel> parse(String text){ ArrayList<Channel> r=new ArrayList<>(); String[] lines=text.replace("\r","").split("\n"); String n="ช่องไม่ระบุ",logo=""; for(int i=0;i<lines.length;i++){String line=lines[i].trim(); if(line.startsWith("#EXTINF")){int comma=line.indexOf(',');n=comma>=0?line.substring(comma+1).trim():"ช่อง";logo=attr(line,"tvg-logo");}else if(!line.isEmpty()&&!line.startsWith("#")){if(line.startsWith("http://")||line.startsWith("https://")){r.add(new Channel(n,logo,line));n="ช่องไม่ระบุ";logo="";}}} return r; }
