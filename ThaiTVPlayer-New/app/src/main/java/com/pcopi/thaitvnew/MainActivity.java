...
public class MainActivity extends AppCompatActivity {
    ...
    @Override protected void onCreate(Bundle b){super.onCreate(b); try { buildUi(); } catch(Throwable t) { setContentView(makeFatal(t)); return; } loadUrl(DEFAULT_PLAYLIST); }
    ...
    private void buildUi(){
        ...
        LinearLayout root=new LinearLayout(this); root.setOrientation(LinearLayout.VERTICAL); drawer.addView(root,new DrawerLayout.LayoutParams(-1,-1));
        LinearLayout bar=new LinearLayout(this); bar.setGravity(Gravity.CENTER_VERTICAL); bar.setBackgroundColor(0xff212121);
        TextView menu=tv("☰",28); menu.setGravity(Gravity.CENTER); bar.addView(menu,new LinearLayout.LayoutParams(dp(54),dp(56))); menu.setOnClickListener(v->drawer.openDrawer(Gravity.START));
        title=tv("Thai TV Player",20); title.setTypeface(null,1); bar.addView(title,new LinearLayout.LayoutParams(0,dp(56),1));
        TextView search=tv("⌕",28); search.setGravity(Gravity.CENTER); bar.addView(search,new LinearLayout.LayoutParams(dp(54),dp(56))); search.setOnClickListener(v->searchDialog()); root.addView(bar);
        ...
        LinearLayout controls=new LinearLayout(this); controls.setPadding(dp(6),dp(3),dp(6),dp(3));
        Button file=btn("ไฟล์ M3U"), net=btn("URL Playlist"), refresh=btn("↻ รีเฟรช"); controls.addView(file,new LinearLayout.LayoutParams(0,dp(48),1)); controls.addView(net,new LinearLayout.LayoutParams(0,dp(48),1)); controls.addView(refresh,new LinearLayout.LayoutParams(0,dp(48),1)); root.addView(controls);
        ...
        status=tv("กำลังเตรียมรายการช่อง…",14); status.setTextColor(0xffffb74d); root.addView(status,new LinearLayout.LayoutParams(-1,dp(40)));
        ...
        DefaultHttpDataSource.Factory http=new DefaultHttpDataSource.Factory().setUserAgent("Mozilla/5.0 (Android) ThaiTVPlayer/3.0").setConnectTimeoutMs(15000).setReadTimeoutMs(25000).setAllowCrossProtocolRedirects(true);
        ...
        player.addListener(new androidx.media3.common.Player.Listener(){@Override public void onPlayerError(PlaybackException e){error("เปิดช่องไม่ได้: "+(e.getErrorCodeName()==null?"ไม่ทราบสาเหตุ":e.getErrorCodeName()));}});
    }
    private void buildDrawer(){
        ...
        addNav(nav,"▣   รายการช่อง",()->showChannels(channels,"รายการช่อง")); addNav(nav,"★   รายการโปรด",()->showChannels(favorites,"รายการโปรด")); addNav(nav,"◷   ประวัติการรับชม",()->showChannels(history,"ประวัติการรับชม"));
    ...
    private void pickFile(){Intent i=new Intent(Intent.ACTION_OPEN_DOCUMENT);i.setType("application/octet-stream");i.addCategory(Intent.CATEGORY_OPENABLE);startActivityForResult(i,PICK_M3U);}
    @Override protected void onActivityResult(int req,int res,Intent data){super.onActivityResult(req,res,data);if(req==PICK_M3U&&res==RESULT_OK&&data!=null&&data.getData()!=null){Uri u=data.getData();executor.execute(()->{try(InputStream in=getContentResolver().openInputStream(u)){if(in==null)throw new IOException("ไม่สามารถอ่านไฟล์");String text=read(in);main.post(()->parseAndShow(text,"ไฟล์ M3U"));}catch(Exception e){main.post(()->error("อ่านไฟล์ไม่ได้: "+e.getMessage()));}});}}
    ...
    private String http(String s)throws Exception{HttpURLConnection c=(HttpURLConnection)new URL(s).openConnection();c.setConnectTimeout(12000);c.setReadTimeout(20000);c.setInstanceFollowRedirects(true);c.setRequestProperty("User-Agent","Mozilla/5.0 (Android) ThaiTVPlayer/3.0");int code=c.getResponseCode();if(code<200||code>=400)throw new IOException("HTTP "+code);try(InputStream in=c.getInputStream()){return read(in);}finally{c.disconnect();}}
