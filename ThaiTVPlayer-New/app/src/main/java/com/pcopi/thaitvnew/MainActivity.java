...
public class MainActivity extends AppCompatActivity {
    ...
    private DrawerLayout drawer; private LinearLayout normalPanel,toolbar,controls; private RecyclerView recycler;
    private TextView status,title; private PlayerView playerView; private FrameLayout playerFrame; private Button closePlayer;
    private ExoPlayer player; private ChannelAdapter adapter;
    ...
    @Override protected void onCreate(Bundle b){super.onCreate(b);getWindow().setStatusBarColor(Color.BLACK);try{buildUi();}catch(Throwable t){setContentView(makeFatal(t));return;}applyOrientation(getResources().getConfiguration().orientation);loadUrl(DEFAULT_PLAYLIST);}
    @Override public void onConfigurationChanged(Configuration c){super.onConfigurationChanged(c);applyOrientation(c.orientation);}
    ...
    private void buildUi(){
        ...
        toolbar=new LinearLayout(this);toolbar.setGravity(Gravity.CENTER_VERTICAL);toolbar.setBackgroundColor(0xff212121);TextView menu=tv("☰",28);menu.setGravity(Gravity.CENTER);toolbar.addView(menu,new LinearLayout.LayoutParams(dp(54),dp(56)));menu.setOnClickListener(v->drawer.openDrawer(Gravity.START));
        title=tv("Thai TV Player",20);title.setTypeface(null,1);toolbar.addView(title,new LinearLayout.LayoutParams(0,dp(56),1));TextView search=tv("⌕",28);search.setGravity(Gravity.CENTER);toolbar.addView(search,new LinearLayout.LayoutParams(dp(54),dp(56)));search.setOnClickListener(v->searchDialog());root.addView(toolbar);
        playerFrame=new FrameLayout(this);playerFrame.setBackgroundColor(Color.BLACK);playerView=new PlayerView(this);playerView.setUseController(true);playerView.setShowBuffering(PlayerView.SHOW_BUFFERING_WHEN_PLAYING);playerView.setBackgroundColor(Color.BLACK);playerFrame.addView(playerView,new FrameLayout.LayoutParams(-1,-1));
        closePlayer=btn("✕");closePlayer.setTextSize(22);closePlayer.setTextColor(Color.WHITE);closePlayer.setBackgroundColor(0xaa222222);FrameLayout.LayoutParams cl=new FrameLayout.LayoutParams(dp(58),dp(52),Gravity.TOP|Gravity.END);cl.setMargins(0,dp(10),dp(10),0);playerFrame.addView(closePlayer,cl);closePlayer.setVisibility(View.GONE);closePlayer.setOnClickListener(v->stopPlayer());root.addView(playerFrame,new LinearLayout.LayoutParams(-1,dp(220)));
        controls=new LinearLayout(this);controls.setPadding(dp(6),dp(3),dp(6),dp(3));Button file=btn("ไฟล์ M3U"),url=btn("URL Playlist"),refresh=btn("↻ รีเฟรช");controls.addView(file,new LinearLayout.LayoutParams(0,dp(48),1));controls.addView(url,new LinearLayout.LayoutParams(0,dp(48),1));controls.addView(refresh,new LinearLayout.LayoutParams(0,dp(48),1));root.addView(controls);file.setOnClickListener(v->pickFile());url.setOnClickListener(v->urlDialog());refresh.setOnClickListener(v->loadUrl(DEFAULT_PLAYLIST));
    ...
    private final DrmSessionManagerProvider clearKeyProvider=mediaItem->{
        ...
        Channel c=(Channel)tag;if(c.clearKey==null||c.clearKey.isEmpty())return DrmSessionManager.DRM_UNSUPPORTED;
        try{byte[] json=buildClearKeyJson(c.clearKey).getBytes(StandardCharsets.UTF_8);return new DefaultDrmSessionManager.Builder().setUuidAndExoMediaDrmProvider(C.CLEARKEY_UUID,FrameworkMediaDrm.DEFAULT_PROVIDER).build(new LocalMediaDrmCallback(json));}catch(Exception e){return DrmSessionManager.DRM_UNSUPPORTED;}
    };
    private String buildClearKeyJson(String pair)throws Exception{String[] p=pair.trim().split(":",2);if(p.length!=2)throw new IllegalArgumentException("ClearKey ต้องเป็น KID:KEY");byte[] kid=hexToBytes(p[0]),key=hexToBytes(p[1]);String k1=Base64.encodeToString(kid,Base64.URL_SAFE|Base64.NO_WRAP|Base64.NO_PADDING),k2=Base64.encodeToString(key,Base64.URL_SAFE|Base64.NO_WRAP|Base64.NO_PADDING);return "{\"keys\":[{\"kty\":\"oct\",\"kid\":\""+k1+"\",\"k\":\""+k2+"\"}],\"type\":\"temporary\"}";}
    private byte[] hexToBytes(String s){String x=s.replace("-","").trim();if((x.length()&1)!=0)throw new IllegalArgumentException("hex ไม่สมบูรณ์");byte[] o=new byte[x.length()/2];for(int i=0;i<o.length;i++)o[i]=(byte)Integer.parseInt(x.substring(i*2,i*2+2),16);return o;}

    private void buildDrawer(){LinearLayout nav=new LinearLayout(this);nav.setOrientation(LinearLayout.VERTICAL);nav.setBackgroundColor(0xff181818);nav.setPadding(0,dp(20),0,dp(12));TextView head=tv("  Thai TV Player",22);head.setTypeface(null,1);head.setTextColor(0xffff9800);nav.addView(head,new LinearLayout.LayoutParams(-1,dp(72)));addNav(nav,"▣   รายการช่อง",()->showChannels(channels,"รายการช่อง"));addNav(nav,"★   รายการโปรด",()->showChannels(favorites,"รายการโปรด"));addNav(nav,"◷   ประวัติการรับชม",()->showChannels(history,"ประวัติการรับชม"));TextView sep=tv("  เพลย์ลิสต์",12);sep.setTextColor(0xff888888);nav.addView(sep,new LinearLayout.LayoutParams(-1,dp(40)));addNav(nav,"＋   เปิดไฟล์ M3U",this::pickFile);addNav(nav,"↗   เปิด URL Playlist",this::urlDialog);addNav(nav,"▶   เปิด URL สตรีม",this::streamDialog);TextView sep2=tv("  อื่นๆ",12);sep2.setTextColor(0xff888888);nav.addView(sep2,new LinearLayout.LayoutParams(-1,dp(40)));addNav(nav,"⚙   ตั้งค่า",this::settingsDialog);addNav(nav,"ⓘ   เกี่ยวกับ",this::aboutDialog);DrawerLayout.LayoutParams lp=new DrawerLayout.LayoutParams(dp(310),-1);lp.gravity=Gravity.START;drawer.addView(nav,lp);}
    ...
    private void pickFile(){Intent i=new Intent(Intent.ACTION_OPEN_DOCUMENT);i.addCategory(Intent.CATEGORY_OPENABLE);i.setType("*/*");i.putExtra(Intent.EXTRA_MIME_TYPES,new String[]{"application/x-mpegURL","application/vnd.apple.mpegurl","audio/mpegurl","audio/x-mpegurl","application/octet-stream","text/plain","*/*"});startActivityForResult(i,PICK_M3U);}
    @Override protected void onActivityResult(int req,int res,Intent data){super.onActivityResult(req,res,data);if(req==PICK_M3U&&res==RESULT_OK&&data!=null&&data.getData()!=null){Uri u=data.getData();executor.execute(()->{try(InputStream in=getContentResolver().openInputStream(u)){if(in==null)throw new IOException("ไม่สามารถอ่านไฟล์");String text=read(in);if(!looksLikeM3u(text))throw new IOException("ไฟล์นี้ไม่ใช่ M3U/M3U8 ที่อ่านได้");main.post(()->parseAndShow(text,"ไฟล์ M3U"));}catch(Exception e){main.post(()->error("อ่านไฟล์ไม่ได้: "+e.getMessage()));}});}}
    ...
    private ArrayList<Channel> parse(String text){ArrayList<Channel> r=new ArrayList<>();String[] lines=text.replace("\uFEFF","").replace("\r","").split("\n");String n="ช่องไม่ระบุ",logo="",clearKey="";for(String raw:lines){String line=raw.trim();if(line.startsWith("#EXTINF")){int comma=line.indexOf(',');n=comma>=0?line.substring(comma+1).trim():"ช่อง";logo=attr(line,"tvg-logo");clearKey="";}else if(line.startsWith("#KODIPROP:")){String prop=line.substring(10).trim();if(prop.startsWith("inputstream.adaptive.license_key="))clearKey=prop.substring(prop.indexOf('=')+1).trim();}else if((line.startsWith("http://")||line.startsWith("https://"))&&!line.startsWith("#")){r.add(new Channel(n,logo,line,clearKey));n="ช่องไม่ระบุ";logo="";clearKey="";}}return r;}
    ...
    private void play(Channel c){if(c==null||c.url==null||c.url.isEmpty()||player==null)return;try{String u=c.url.trim();MediaItem.Builder mb=new MediaItem.Builder().setUri(u).setTag(c);String low=u.toLowerCase(Locale.ROOT);if(low.contains(".m3u8")||low.contains("m3u8?"))mb.setMimeType(MimeTypes.APPLICATION_M3U8);else if(low.contains(".mpd")||low.contains("mpd?"))mb.setMimeType(MimeTypes.APPLICATION_MPD);if(c.clearKey!=null&&!c.clearKey.isEmpty())mb.setDrmConfiguration(new MediaItem.DrmConfiguration.Builder(C.CLEARKEY_UUID).setMultiSession(false).build());player.stop();player.clearMediaItems();player.setMediaItem(mb.build());player.prepare();player.play();addHistory(c);title.setText(c.name);status.setText("กำลังเปิด: "+c.name);status.setTextColor(0xffffb74d);}catch(Exception e){error("เปิดช่องไม่ได้: "+e.getMessage());}}
