...
public class BiometricVault {
    ...
    private static final String ALIAS="PrivateAppBiometricKey";
    private static final String PREF="biometric";
    ...
    public boolean canUse(){
        ...
        try{return context.getPackageManager().hasSystemFeature("android.hardware.fingerprint") || context.getPackageManager().hasSystemFeature("android.hardware.biometrics");}
        catch(Exception e){return false;}
    }
    private SecretKey key()throws Exception{
        KeyStore ks=KeyStore.getInstance(KS);ks.load(null);return (SecretKey)ks.getKey(ALIAS,null);
    }
    private void createKey()throws Exception{
        KeyStore ks=KeyStore.getInstance(KS);ks.load(null);if(ks.containsAlias(ALIAS))ks.deleteEntry(ALIAS);
    ...
    private Cipher cipher(int mode,byte[] iv)throws Exception{
        ...
        if(iv==null)c.init(mode,key());else c.init(mode,key(),new GCMParameterSpec(128,iv));
        return c;
    }
    public void enable(final Activity a,final String master,final Runnable ok,final Runnable fail){
        if(!canUse()){fail.run();return;}
    ...
    public void disable(){
        ...
        try{KeyStore ks=KeyStore.getInstance(KS);ks.load(null);if(ks.containsAlias(ALIAS))ks.deleteEntry(ALIAS);}catch(Exception ignored){}
    }
    public void unlock(final Activity a,final Runnable ok,final Runnable fail){
        if(!enabled()){fail.run();return;}
        try{byte[] iv=Base64.decode(prefs.getString(IV,""),Base64.NO_WRAP);final Cipher c=cipher(Cipher.DECRYPT_MODE,iv);authenticate(a,c,new Result(){public void success(Cipher done){try{String m=new String(done.doFinal(Base64.decode(prefs.getString(DATA,""),Base64.NO_WRAP)),StandardCharsets.UTF_8);if(m.isEmpty())throw new Exception();okWithMaster(ok,m);}catch(Exception e){fail.run();}}public void error(){fail.run();}});}catch(Exception e){fail.run();}
    }
    private void okWithMaster(Runnable ok,String master){if(ok instanceof MasterCallback)((MasterCallback)ok).run(master);else ok.run();}
    public interface MasterCallback extends Runnable{void run(String master);}
    private interface Result{void success(Cipher c);void error();}
    private void authenticate(Activity a,final Cipher cipher,final Result result){
        if(Build.VERSION.SDK_INT<28){result.error();return;}
        Executor ex=a.getMainExecutor();
        ...
        b.setNegativeButton("ใช้ Password",ex,new DialogInterface.OnClickListener(){public void onClick(DialogInterface d,int w){result.error();}});
