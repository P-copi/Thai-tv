...
public class VaultStore {
    ...
    public String encryptedBackup(String master)throws Exception{return "MPV1\n"+salt()+"\n"+p.getString("data","");}
    public void restoreEncryptedBackup(String backup,String master)throws Exception{String[] x=backup.split("\\n",3);if(x.length!=3||!"MPV1".equals(x[0]))throw new Exception("ไฟล์ Backup ไม่ถูกต้อง");String newSalt=x[1];String plain=VaultCrypto.decrypt(x[2],master,newSalt);new JSONArray(plain);p.edit().putString("salt",newSalt).putString("check",VaultCrypto.encrypt("VAULT_OK",master,newSalt)).putString("data",x[2]).apply();}
