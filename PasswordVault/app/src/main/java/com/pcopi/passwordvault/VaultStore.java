...
public class VaultStore {
    ...
    private static volatile String sessionMaster="";
    ...
    public boolean unlock(String master){try{boolean ok="VAULT_OK".equals(VaultCrypto.decrypt(p.getString("check",""),master,salt()));if(ok)sessionMaster=master;return ok;}catch(Exception e){return false;}}
    public List<Entry> entries(String master){
        List<Entry> out=new ArrayList<>();
        try{
            String m=effective(master);
            JSONArray a=new JSONArray(VaultCrypto.decrypt(p.getString("data",""),m,salt()));
    ...
    public void saveEntries(String master,List<Entry> list)throws Exception{
        String m=effective(master);if(m.isEmpty())throw new Exception("กรุณาเข้าสู่ระบบใหม่");
        ...
        if(!p.edit().putString("data",VaultCrypto.encrypt(a.toString(),m,salt())).commit())throw new Exception("บันทึกข้อมูลไม่สำเร็จ");
    }
    public String encryptedBackup(String master)throws Exception{
        String m=effective(master);if(m.isEmpty()||!unlock(m))throw new Exception("Master Password ไม่ถูกต้อง");
        return "MPV2\n"+salt()+"\n"+p.getString("data","");
    }
    public void restoreEncryptedBackup(String backup,String backupPassword,String currentMaster)throws Exception{
        ...
        String target=effective(currentMaster);if(target.isEmpty())target=backupPassword;
